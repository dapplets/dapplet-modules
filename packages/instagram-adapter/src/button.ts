...
export class Button implements IWidget<IButtonState> {
    ...
    public mount() {
        ...
        if (this.insPointName === 'POST') {
            const htmlString = `
                <span class="_15y0l">
                    <button class="wpO6b" type="button" style="padding: 0; margin: 0; border: 0; background: inherit;">
            ...
            const htmlString = `
                <span style="margin-left: 8px;" class="QzzMF Igw0E IwRSH eGOV_ ui_ht bPdm3">
                    <button class="sqdOP L3NKy _8A5w5 ZIAjV">
                        ${img ? `<img style="position: relative; top: 2px; margin-right: 4px;" height="14" src="${img}" />` : ''}
                        ${label}
                    </button>
                </span>
            `;
